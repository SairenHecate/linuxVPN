...
int main(int argc, char **argv)
{
	...
	while (1)
	{
			...
			if (!fork()) //fork创建子进程 返回值为0的是子进程 
		{
				...
				while (1)
				{
					memset(ucRecvBuf,0,4096);
					/* 接收客户端发来的数据并显示出来 */
					iRecvLen = recv(iSocketClient, ucRecvBuf, 4096, 0);
					if (iRecvLen <= 0)
					{
					...
					else
					{
						...
						for(i=0;i<iRecvLen;i++){
							printf("%2X ",ucRecvBuf[i]);
						}
						printf("\n");
						unsigned char sendbuff[8]={};
						memcpy(sendbuff,"hello!!",8);
						send(iSocketClient, sendbuff, 8, 0);//打印的同时发送一条数据给连接的客户端
						printf("send over!\n");
					}
				}				
			}
